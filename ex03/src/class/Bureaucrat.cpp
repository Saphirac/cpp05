...
{
	if (DEBUG)
		std::cout << "Bureaucrat executeForm member function called\n";
	if (toExecute.getIsSigned() == false)
	{
		std::cout << this->_name << " couldn't execute " << toExecute.getName()
		<< " because form is not signed.\n";
	}
	else if (this->_grade > toExecute.getExecGrade())
	{
		std::cout << this->_name << " couldn't execute " << toExecute.getName()
		<< " because grade is insuffisant.\n";
	}
