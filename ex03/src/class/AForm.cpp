...
void	AForm::setSigned(bool const &input)
{
	if (DEBUG)
		std::cout << "Form setSigned member function called\n";
	this->_isSigned = input;
}
...
{
	if (DEBUG)
		std::cout << "AForm AlreadySignedException thrown\n";
	return "AForm is alreadysigned\n";
}

char const *AForm::NotSignedException::what(void) const throw()
{
	if (DEBUG)
		std::cout << "AForm NotSignedException thrown\n";
	return "AForm isn't signed for execution\n";
}
